...
L["options:alpha:use-target-alpha-if-not-target-selected"] = "Если не цель не выбрана, отобразить ауры с прозрачностью цели"
...
L["options:apps:dr"] = "Включить отображение убывающей отдачи (бета)"
L["options:apps:dr:pve"] = "ПВЕ (при оглушении)"
...
L["options:general:instance-types"] = "Установить видимость обратных отсчетов в разных типах локаций"
...
L["options:general:show-on-target-only"] = "Показывать ауры только на панели имени цели"
...
L["options:size-and-position:custom-sorting:tooltip"] = "Правила: - код должен быть не переименованной функцией с 2 аргументами. Эти аргументы-таблицы, представляющие собой сравниваемые ауры - эта функция должна вернуть true(истина), если первая аура должна быть помещена перед второй аурой и false(ложь) в ином случае - сортировка производится довольно часто, так что не нагружайте функцию сортировки слишком сильно - не изменяйте содержимое таблицы ауры, кроме тех случаев, когда вы ДЕЙСВИТЕЛЬНО знаете, что делаете - внимательно проверьте любой код, который вы получили от неизвестных источников Содержимое таблицы ауры: - aura.duration - содержит длительность ауры в секундах. Если аура постоянная, то значение этого поля равно 0(ноль). (type: number) - aura.expires - время, когда аура закончится. Вы можете сравнить его с GetTime(). Если аура постоянная, то значение этого поля равно 0(ноль). (type: number) - aura.stacks - количество стаков (type: number) - aura.spellID - ID(ИД) ауры (type: number) - aura.spellName - имя ауры (type: string) Встроенные функции сортировки (result is a boolean value):  - local result = sort_time(aura1, aura2) - сортировать по оставшемуся времени ауры - local result = sort_size(aura1, aura2) - сортироваться по размеру иконки"
...
L["options:size-and-position:keep-aspect-ratio:tooltip"] = "Если эта опция включена и ширина и высота иконки не равны, то текстура заклинания будет обрезана так, чтобы сохранить изначальные пропорции изображения"
