...
GBB_GetDungeons = function(msg)
	...
	-- DEFAULT_CHAT_FRAME:AddMessage('parts:'..table.tostring(parts))
...
GBB_PhraseMessage = function(msg, name, guid, channel)
    ...
    local updated = false
    -- DEFAULT_CHAT_FRAME:AddMessage('11111111111111111')
    ...
    -- DEFAULT_CHAT_FRAME:AddMessage('2222222222222222')
    if updated == true then
        GBB_UpdateList()
        return
    end

    -- DEFAULT_CHAT_FRAME:AddMessage('3333333333333333')

    --flm RFD need healer and 3 dps
    local dungeonList, isGood, isBad, wordcount = GBB_GetDungeons(msg)

    -- DEFAULT_CHAT_FRAME:AddMessage('---dungeonList:' .. table.tostring(dungeonList) .. ',isGood:' .. tostring(isGood) .. ',isBad:' .. tostring(isBad) .. ',wordcount:' .. tostring(wordcount))
	...
	-- DEFAULT_CHAT_FRAME:AddMessage('GBB_RequestList:'..table.tostring(GBB_RequestList))

    if wordcount >= 1 then
        for dungeon, id in pairs(dungeonList) do
            local index = 0
            if id == true and dungeon ~= nil then
				-- DEFAULT_CHAT_FRAME:AddMessage('111111111111111111')
				...
				-- DEFAULT_CHAT_FRAME:AddMessage('22222222222222222')
                if index == 0 then
					...
					-- DEFAULT_CHAT_FRAME:AddMessage('GBB_FilterDungeon(req.dungeon):'..dungeon..':'..tostring(GBB_FilterDungeon(dungeon)))
...
GBB_CHAT_MSG_CHANNEL = function(msg, name, guid, channel)
    -- DEFAULT_CHAT_FRAME:AddMessage('---msg:' .. msg .. ',name:' .. name .. ',guid:' .. guid .. ',channel:' .. channel)
