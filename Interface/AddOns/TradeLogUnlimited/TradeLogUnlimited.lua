...
function TradeLogUnlimited_OnLoad(self)
	...
	cb:SetPoint("BOTTOMLEFT", "TradeFrame", "BOTTOMLEFT", 0, 0);
	cb:SetWidth(26);
	cb:SetHeight(26);
    ...
    menu:SetPoint('BOTTOMLEFT', cb, 70, -2)
...
function TradeLogUnlimited_GetTradeList(money, items, enchant, moneyText, countOnly)
	...
	for i=1,6 do
		if(items[i]) then 
			if items_lookup[items[i].itemLink] then	
				items_lookup[items[i].itemLink].tCount = items[i].numItems + items_lookup[items[i].itemLink].tCount
			else
				items_lookup[items[i].itemLink] = items[i]
				items_lookup[items[i].itemLink].tCount = items[i].numItems
			end
		end
	end
	for k, v in pairs(items_lookup) do
	...
	for i=1,6 do
		if(temp_items[i]) then
			count = count + 1
			local tmp = temp_items[i].itemLink;
			if(temp_items[i].numItems>1) and temp_items[i].tCount ~= nil then
				tmp = tmp.."x"..temp_items[i].tCount;
			end
	...
	if(trade.result == "complete") then
		...
		if(#playerList==0 and #targetList==0) then
			...
			if(plain) then
				...
				func(string.gsub(TRADE_LOG_SUCCESS, "%%t", whoLink)..detailString..":", 1, 1, 1);
				if(#playerList>0) then
					func(""..TRADE_LOG_HANDOUT.." "..table.concat(playerList, ", "), 0, 0.682352941, 0.937254902);
				end
				if(#targetList>0) then
					func(""..TRADE_LOG_RECEIVE.." "..table.concat(targetList, ", "), 0, 0.6, 0);
				end
			end
		end
