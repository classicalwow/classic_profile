...
local function InitVariables_Once() -- {{{
    ...
    D.Revision = "f1d5853"; -- not used here but some other add-on may request it from outside
    D.date = "2021-11-11T17:02:56Z";
    D.version = "2.7.8.4";

    if D.date ~= "@project".."-date-iso@" then
        -- 1636650176 doesn't work
...
function D:VersionWarnings(forceDisplay) -- {{{
    ...
    if DcrMaxTOC < T._tocversion then
        ...
        elseif time() - self.db.global.TocExpiredDetection > 3600 * 24 * 40 then -- if more than 40 days elapsed since the detection
            ...
            if time() - self.db.global.LastExpirationAlert > 48 * 3600 or forceDisplay then
                ...
                T._ShowNotice ("|cff00ff00Decursive version: 2.7.8.4|r\n\n" .. "|cFFFFAA66" .. L["TOC_VERSION_EXPIRED"] .. "|r");
    ...
    if (("2.7.8.4"):lower()):find("beta") or ("2.7.8.4"):find("RC") or ("2.7.8.4"):find("Candidate") or alpha then
        ...
        if self.db.global.NonRelease ~= "2.7.8.4" then
            self.db.global.NonRelease = "2.7.8.4";
...
T._LoadedFiles["DCR_init.lua"] = "2.7.8.4";
