...
local defConfig = {
	...
	ShiftFocus = WOW_PROJECT_ID ~= WOW_PROJECT_CLASSIC,
...
function UF.create_extra_power0(U, unit, portraitPos, perOfs)
	...
	if pb then
		...
		extra_power0:SetBackdropColor(0.0, 0.0, 0.0, 0.0);
		extra_power0:SetBackdropBorderColor(0.0, 0.0, 0.0, 1.0);
...
function UF.guess_tick_is_mana_regen(U, diff)
...
function UF.createTargetFrame(U, unit, targetPos, set)
	...
	T:SetBackdropColor(0.0, 0.0, 0.0, 1.0);
	T:SetBackdropBorderColor(0.0, 0.0, 0.0, 1.0);
	...
	function T.UNIT_HEALTH(self, event, unitId)
		if unit == unitId then
			self:UpdateHealth();
		end
	end
	function T.UNIT_MAXHEALTH(self, event, unitId)
		if unit == unitId then
			self:UpdateHealth();
		end
	end
	function T.UNIT_POWER_UPDATE(self, event, unitId, powerToken)
		if unit == unitId then
			self:UpdatePower();
		end
		-- "MANA", "RAGE", "ENERGY", "FOCUS", "HAPPINESS", "RUNIC_POWER", "HOLY_POWER"
	end
	function T.UNIT_DISPLAYPOWER(self, event, unitId)
		if unit == unitId then
			self:UpdatePowerType();
		end
	end
	...
	-- UF.regUnitEvent(T, target, "UNIT_HEALTH", "UNIT_MAXHEALTH", "UNIT_POWER_UPDATE", "UNIT_DISPLAYPOWER");
...
function UF.coverFontString(uf, covered, layoutLevel, subLayoutLevel)
	if not covered or type(covered) ~= 'table' then
		return;
	end
	local origLevel, origSubLevel = covered:GetDrawLayer();
...
local function init()
	do	--	player & pet
		...
		if WOW_PROJECT_ID == WOW_PROJECT_BURNING_CRUSADE_CLASSIC then
			PU.status:SetTexCoord(1.0, 0.09375, 0.0, 0.78125);
		...
		UF.regEvent(PU, "PLAYER_REGEN_ENABLED", "PLAYER_REGEN_DISABLED", "GROUP_ROSTER_UPDATE", "UPDATE_CHAT_COLOR");
	...
	do	--	party
		...
		for i = 1, 4 do
			local U = UF.hookUnitFrame(_G["PartyMemberFrame" .. i], 'party' .. i, "LEFT", nil, nil, { true, true, true, nil, nil, false, 10, 1.0, }, true, true, { 50, 1.0, }, nil, true);
			U.class:SetScale(0.75);
			U.class:ClearAllPoints();
			U.class:SetPoint("TOPLEFT", 40, -6);
	...
	--	setting
		...
		if WOW_PROJECT_ID ~= WOW_PROJECT_CLASSIC and alaUnitFrameSV.ShiftFocus then
...
do
	...
	local function CreateSlider(specific, key, label_text, minRange, maxRange, stepSize, i, j)
		...
		texture:SetPoint("TOPLEFT", configFrame, "TOPLEFT", (i - 1) * col_width + 20, - row_height * (j - 1) - 20);
	...
	local function CreateDrop(specific, key, labelText, i, j, data)
		...
		label:SetPoint("TOPLEFT", configFrame, "TOPLEFT", (i - 1) * col_width + 20, - row_height * (j - 1) - 20);
	...
	local function CreateCheckBox(specific, key, label, i, j)
		...
		cb:SetPoint("TOPLEFT", configFrame, "TOPLEFT", (i - 1) * col_width + 20, - row_height * (j - 1) - 20);
	...
	local function CreateValueBox(specific, key, label, i, j)
		...
		valueBox:SetBackdropColor(0.0, 0.0, 0.0, 0.5);
		valueBox:SetBackdropBorderColor(0.3, 0.3, 0.3, 0.8);
		...
		valueBox:SetPoint("TOPLEFT", configFrame, "TOPLEFT", (i - 1) * col_width + 20, - row_height * (j - 1) - 20);
	...
	function UF.PLAYER_ENTERING_WORLD(self, event)
		...
		if WOW_PROJECT_ID ~= WOW_PROJECT_CLASSIC then
			CreateCheckBox(false, "ShiftFocus", L["ShiftFocus"], 2, 9);
		end
