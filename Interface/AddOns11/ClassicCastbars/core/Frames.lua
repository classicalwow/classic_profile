...
function addon:HideCastbar(castbar, unitID, skipFadeOut)
    ...
    if cast then
        if cast.isInterrupted or cast.isFailed then
            ...
            local ____t = self.db[self:GetUnitType(unitID)].statusColorFailed;
            if ____t ~= nil then
                castbar:SetStatusBarColor(unpack(____t))
            end
...
local function ColorPlayerCastbar(db)
    ...
    if CastingBarFrame.Background then
