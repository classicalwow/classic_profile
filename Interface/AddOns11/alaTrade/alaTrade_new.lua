...
do	--	MAIN
	...
	do	--	hook tooltip
		-- price
		local function hook_tooltip(tip, id, num)
		...
		function NS.hook_tooltip()
			...
			hooksecurefunc(GameTooltip, "SetTrainerService",
				function(tip, service)
					local _, itemstring = GameTooltip:GetItem();
	...
	do	--	hook Auction
		...
		do	-- hook Blizzard_AuctionUI
			...
			local function hook_AuctionFrameBrowse()
				...
				--
				BrowseDisplay = CreateFrame("FRAME", nil, AuctionFrameBrowse);
			...
			local function hook_AuctionFrameAuctions()
				...
				--
				AuctionDisplay = CreateFrame("FRAME", nil, AuctionFrameAuctions);
	...
	do	--	UI
		...
		local function button_OnClick(self)
			...
			if data_index <= #list then
				...
				local info = cache[id];
				if info then
					info[index_link] = info[index_link] or select(2, GetItemInfo(id));
