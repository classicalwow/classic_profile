...
hooksecurefunc (GameTooltip, "SetAuctionSellItem",
  function (tip, type, index)
    local _, _, num = GetAuctionSellItemInfo();
    Atr_ShowTipWithPricing (tip, num);
  end
);
...
hooksecurefunc (GameTooltip, "SetSendMailItem",
  function (tip, index)
    local _, _, _, num = GetSendMailItem(index);
    if num then
      Atr_ShowTipWithPricing (tip, num);
    end
  end
...
function Atr_ShowTipWithPricing (tip, num)
  ...
  if ilink and not isIsBlank(ilink) and showSomething then
    ...
    local vendorPrice, auctionPrice, dePrice, auctionWeekPrice, auctionMonthPrice = Atr_STWP_GetPrices (itemLink, nil, itemVendorPrice, itemName, classID, itemRarity, itemLevel);

    local shift = IsShiftKeyDown();
    local showSingle = (AUCTIONATOR_SHIFT_TIPS == 1 and shift) or (AUCTIONATOR_SHIFT_TIPS == 2 and not shift);
    num = tonumber(num)
    local showStack = not showSingle and num ~= nil and num > 1;
    showSingle = showSingle or AUCTIONATOR_SHIFT_TIPS == 3;
    if AUCTIONATOR_SHIFT_TIPS ~= 4 then
      -- vendor info
      if showSingle then
        Atr_STWP_AddVendorInfo (tip, "", vendorPrice, false)
      end
      if showStack and vendorPrice ~= nil then
        Atr_STWP_AddVendorInfo (tip, "|cFFAAAAFF x" .. num .. "|r", vendorPrice * num, false)
      end

      -- auction info
      if showSingle then
        Atr_STWP_AddAuctionInfo (tip, "", itemLink, auctionPrice, false)
      end
      if showStack and auctionPrice ~= nil then
        Atr_STWP_AddAuctionInfo (tip, "|cFFAAAAFF x" .. num .. "|r", itemLink, auctionPrice * num, false)
      end
    end

    -- auction summary
    if auctionPrice and auctionWeekPrice ~= nil and AUCTIONATOR_A_WEEK_TIPS == 1 then
      local left = ZT("Auction").." week"
      local right = "|cFFFFFFFF"
    ...
    -- disenchanting info
    Atr_STWP_AddBasicDEInfo (tip, "", dePrice, true)
...
function Atr_ShowReagentTooltip(tip, craftType, index, reagent)do return end
  local showSomething = AUCTIONATOR_A_TIPS == 1 or AUCTIONATOR_V_TIPS == 1 or AUCTIONATOR_D_TIPS == 1
