	...
	if (doUpdateMinimapButton and (usingPanel or relativeTo and relativeTo:GetName() == "LibDBIcon10_NovaWorldBuffs")) then
		...
		if (NWB.isTBC) then
			...
			if (NWB.data.tbcPD and NWB.data.tbcPDT and GetServerTime() - NWB.data.tbcPDT < 86400) then
				...
				tooltip:AddLine(NWB.chatColor .."|cFFFF6900日常|r |cFF9CD6DE(|r" .. texture .. "|cFF9CD6DE)|r 未知.");
			end
		end
...
function NWB:createDmfMarkers()
	...
	obj.tooltip.fs:SetPoint("CENTER", 0, 0);
	obj.tooltip.fs:SetFont(NWB.regionFont, 14);
	obj.tooltip.fs:SetText("|Cff00ff00暗月马戏团");
	obj.tooltip:SetWidth(obj.tooltip.fs:GetStringWidth() + 18);
	obj.tooltip:SetHeight(obj.tooltip.fs:GetStringHeight() + 12);
	obj:SetScript("OnEnter", function(self)
		obj.tooltip:Show();
...
NWBbuffListFrame.fs2:SetText("|cffffff00鼠标悬停角色名上显示更多信息");
...
NWBbuffListDragFrame.tooltip.fs:SetText("按住拖动");
...
NWBbuffListFrameTimersButton:SetText("计时器");
...
function NWB:createShowStatsButton()
	if (not NWB.showStatsButton) then
		...
		NWB.showStatsButton.tooltip = "显示你获得buff的次数";
	...
	if (not NWB.showStatsAllButton) then
		...
		NWB.showStatsAllButton.tooltip = "显示全部有buff数据的小号? (数据必须开启).";
...
function NWB:recalcBuffListFrame()
	...
	if (not foundChars) then
		...
		NWB:insertBuffsLineFrameString("|cffffff00没有找到有BUFF的角色.");
	else
		NWBbuffListFrame.fs2:SetText("|cffffff00鼠标悬停到角色名上获取额外信息");
	end
...
NWBlayerDragFrame.tooltip.fs:SetText("按住拖动");
...
function NWB:createCopyFormatButton()
	if (not NWB.copyDiscordButton) then
		...
		NWB.copyDiscordButton.tooltip = "设置粘贴到discordl论坛的文本格式? (增加颜色等)";
...
NWBLayerMapDragFrame.tooltip.fs:SetText("按住拖动");
...
function NWB:recalcLayerMapFrame()
	...
	if (not NWB.data.layers or type(NWB.data.layers) ~= "table" or not next(NWB.data.layers)) then
		NWBLayerMapFrame.EditBox:Insert("|cffFFFF00服务器重启后还没有地图区域被映射.|r\n");
	else
		local count = 0;
		for k, v in NWB:pairsByKeys(NWB.data.layers) do
			count = count + 1;
...
NWBVersionFrame.fs:SetText("|cFFFFFF00登录后查看公会版本|r");
...
NWBVersionDragFrame.tooltip.fs:SetText("按住拖动");
...
function NWB:recalcVersionFrame()
	...
	if (not IsInGuild()) then
		NWBVersionFrame.EditBox:Insert("|cffFFFF00你没有公会, 这个命令只有公会成员才能显示.|r\n");
	else
		GuildRoster();
		local numTotalMembers = GetNumGuildMembers();
		local onlineMembers = {};
...
function NWB:createNaxxMarkers()
	...
	obj.tooltip.fs:SetText("|CffDEDE42纳克萨玛斯");
...
local iwtKeybind = "";
NWBDmfFrame.fs2:SetText("当前和目标按键绑定交互: |cffffa500" .. iwtKeybind);
...
NWBDmfFrame.fs3:SetText("|cffffff00选中赛格并处于交互范围内\n开始前.");
...
function NWB:updateInteractBindText()
	...
	NWBDmfFrame.fs2:SetText("当前和目标按键绑定交互:\n|cffffa500" .. iwtKeybind);
end
...
NWBDmfDragFrame.tooltip.fs:SetText("按住拖动");
...
NWBDmfFrameClose:SetScript("OnClick", function(self, arg)
	...
	NWBDmfFrame:Hide();
	NWB:print("关闭DMF助手, 要重新打开它，请远离赛格并重新与其交互 (这个窗口可以在设置中关闭/nwb config).");
end)
--Adjust the X texture so it fits the entire frame and remove the empty clickable space around the close button.
--Big thanks to Meorawr for this.
...
NWBDmfFrameStopStuckButton:SetText("取消");
...
NWBDmfFrameStartLogoutButton:SetText("开始注销");
...
NWBDmfFrameStopLogoutButton:SetText("取消");
...
NWBChangeInteractKeybindButton:SetText("更改绑定");
...
NWBChangeInteractKeybindButton:SetScript("OnClick", function(self, arg)
	...
	for i = 1, 20 do
		if (_G["KeyBindingFrameKeyBinding" .. i] and _G["KeyBindingFrameKeyBinding" .. i].description
			...
			_G["KeyBindingFrameKeyBinding" .. i].description:SetText("与目标交互  |CFFFFFFFF<- HERE");
		end
	end
end)
...
NWBChangeInteractKeybindButton.tooltip.fs:SetText("|CffDEDE42打开按键绑定菜单.\n按键绑定 -> 目标 -> 与目标交互");
...
function NWB:createDmfHelperButtons()
	if (not NWB.dmfChatCountdown) then
		...
		NWBDMFChatCountdownText:SetText("群聊倒计时");
	...
	if (not NWB.dmfAutoResButton) then
		...
		NWBDMFAutoResButtonText:SetText("自动复活");
	...
	if (not NWB.dmfAutoResSlider) then
		...
		NWBDMFAutoResSliderText:SetText("自动复活剩余秒数");
