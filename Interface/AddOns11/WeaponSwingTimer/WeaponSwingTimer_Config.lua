...
addon_data.config.InitializeVisuals = function()
    ...
    panel.config_melee_panel.name = '近战攻击条设置'
...
addon_data.config.TextFactory = function(parent, text, size)
    ...
    text_obj:SetFont("Fonts/ARKai_T.ttf", size)
...
addon_data.config.CreateConfigPanel = function(parent_panel)
    ...
    local settings = character_player_settings
    -- Title Text
    panel.title_text = addon_data.config.TextFactory(panel, "平砍攻击计时条", 20)
    ...
    -- Guidance Text
    panel.guidance_text = addon_data.config.TextFactory(panel, "看左边的标签，所有设置都先点左边的标签", 16)
