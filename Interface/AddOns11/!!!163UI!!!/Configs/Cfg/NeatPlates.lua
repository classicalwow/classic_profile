    ...
    toggle = function(name, info, enable, justload)
        if enable then
            if justload and IsLoggedIn() then
                -- if Export163_OnNewNameplate then
                ...
                local index = 1;
                while true do
                    local p = _G["NamePlate" .. index];
                    if p then
                        Export163_OnNewNameplate(p);
                    else
                        break;
                    end
                    index = index + 1;
                end
                EnableAddOn("NeatPlatesWidgets")
                LoadAddOn("NeatPlatesWidgets")
                EnableAddOn("NeatPlatesHub");
                LoadAddOn("NeatPlatesHub");
            end
