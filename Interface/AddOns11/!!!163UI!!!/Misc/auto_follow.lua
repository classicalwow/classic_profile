...
local function start_follow(unit, name)
    ...
    if ec then
        ...
        info:SetText(string.format("正在跟随[\124cff%.2x%.2x%.2x%s\124r]", color.r * 255, color.g * 255, color.b * 255, name));
    else
        info:SetText("正在跟随[" .. name .. "]")
    end
end

local FOLLOW_UNIT_TEXT = { enUS = "Keep following unit", zhCN = "一直跟随目标", zhTW = "一直跟随目标", };

alaPopup.add_meta("FOLLOW_UNIT",
    {
        FOLLOW_UNIT_TEXT[locale] or FOLLOW_UNIT_TEXT.enUS, 
...
local UnitPopupButtonsExtra = {
    ["FOLLOW_UNIT"] = { enUS = "Keep following unit", zhCN = "一直跟随目标", zhTW = "一直跟随目标", },
