
DataStore_MailsDB = {
	["profileKeys"] = {
		["顶级程序员 - 鱼人摩洛克"] = "顶级程序员 - 鱼人摩洛克",
		["特变电工 - 鱼人摩洛克"] = "特变电工 - 鱼人摩洛克",
		["Javascript - 鱼人摩洛克"] = "Javascript - 鱼人摩洛克",
	},
	["global"] = {
		["Characters"] = {
			["Default.鱼人摩洛克.特变电工"] = {
				["lastUpdate"] = 1646236498,
				["Mails"] = {
					{
						["lastCheck"] = 1646236498,
						["sender"] = "客服支持",
						["subject"] = "举报受理通知",
						["returned"] = true,
						["money"] = 0,
						["text"] = "尊敬的艾泽拉斯勇士： \n您好! \n我们已经收到了您于 2022-03-01 21时,在游戏中对玩家 黑龙费尔默-修*的举报，我们将持续对该角色进行监控，推荐您关注并绑定微信公众号“暴雪游戏服务中心”获得结果通知，如有多个举报角色，可登录官方暴雪游戏安全中心-个人中心我的举报查看所有举报详情。\n此致 \n网 易 暴 雪 游 戏 客 服 团队",
						["icon"] = 135724,
						["daysLeft"] = 364.2047119140625,
					}, -- [1]
				},
				["lastVisitDate"] = "2022/03/02 23:54",
			},
			["Default.鱼人摩洛克.Javascript"] = {
				["lastUpdate"] = 1645540439,
				["Mails"] = {
					{
						["icon"] = 134327,
						["sender"] = "银脉长者",
						["subject"] = "",
						["returned"] = false,
						["money"] = 0,
						["text"] = "感谢你来看望我，Javascript。我很高兴，因为我们还没有被遗忘。",
						["lastCheck"] = 1645540439,
						["daysLeft"] = 15.25062465667725,
					}, -- [1]
					{
						["icon"] = 134327,
						["sender"] = "铜须长者",
						["subject"] = "",
						["returned"] = false,
						["money"] = 0,
						["text"] = "感谢你来看望我，Javascript。我很高兴，因为我们还没有被遗忘。",
						["lastCheck"] = 1645540439,
						["daysLeft"] = 15.27971076965332,
					}, -- [2]
					{
						["icon"] = 134327,
						["sender"] = "星纹长者",
						["subject"] = "",
						["returned"] = false,
						["money"] = 0,
						["text"] = "这件小小的礼物不会给你答案，但也许它可以让你感到快乐。",
						["lastCheck"] = 1645540439,
						["daysLeft"] = 15.30505752563477,
					}, -- [3]
				},
				["lastVisitDate"] = "2022/02/22 22:33",
			},
		},
	},
}
